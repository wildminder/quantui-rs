...
fn dump_headers() {
    ...
    for (n, i) in theirs.header().iter() {
        ...
        println!(
            ...
            i.shape,
            i.data_offsets,
            o.map(|oi| format!("dtype={} shape={:?} offs={:?}", oi.dtype_raw, oi.shape, oi.data_offsets))
    ...
    println!(
        ...
        u64::from_le_bytes(std::fs::read(dir.join("output.safetensors")).unwrap()[0..8].try_into().unwrap())
    );
    ...
    for (i, (x, y)) in a.iter().zip(&b).enumerate() {
        if x != y {
            ...
            println!("context ours: {:02x?}", &a[i.saturating_sub(16)..(i + 16).min(a.len())]);
            println!("context gold: {:02x?}", &b[i.saturating_sub(16)..(i + 16).min(b.len())]);
            break;
        }
    }
}
